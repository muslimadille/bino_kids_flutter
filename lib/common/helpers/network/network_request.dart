...
class NetworkRequest with NetworkExceptionHandler {
  ...
  static Dio _createDioClient() {
    ...
    // ✅ دعم الأجهزة القديمة (TLS < Android 12)
  ...
  Future<Response> sendAppRequest({
    required NetworkRequestModel networkParameters,
    required NetworkExceptionModel exceptionParameters,
  }) async {

    if (networkParameters.showProgress ?? false) EasyLoading.show();
    if ((networkParameters.showProgress ?? false) ||
        (networkParameters.dismissProgress ?? false)) {
      canBack = false;
    }

    log("******Request************(serviceId:${networkParameters.apiCode})"
        "\nheader : ${networkParameters.header}"
    ...
    try {
      final response = await dio.request(
        ...
        options: Options(
          ...
          method: (networkParameters.networkType ?? NetworkRequestEnum.get)
              .name
              .toUpperCase(),
      ...
      if (response.data["status"].toString() == "0" &&
          (exceptionParameters.showError ?? false)) {
        CustomSnakbar().appSnackBar(
      ...
      final String? errorMessage = dioErrorHandler(
        dioError: dioError,
        exceptionModel: exceptionParameters,
      );
      ...
      if (networkParameters.rethrowError ?? false) rethrow;
